...
std::string to_timestamp(int64_t t){
    ...
    char buf[32];
    snprintf(buf,sizeof(buf),"%02d:%02d.%02d",(int) min,(int) sec,(int) msec);
...
bool whisper_params_parse(int argc, char ** argv, whisper_params &params){
    for(int i=1;i< argc;i++){
        ...
        else if (arg == "-t"    || arg == "--thread") {params.n_threads  = std::stoi(argv[++i]);}
...
// print some info about the processing
    ...
    while (is_running){
        ...
        // run the interface 
        {
            static std::vector<std::string> question;    
            ...
            if (whisper_full(ctx, wparams, pcmf32.data(), pcmf32.size()) != 0) {     // Here transfer audio to str
                fprintf(stderr, "%s: failed to process audio\n", argv[0]);
                return 6;
            }
            else{
                // N: after audio transfer to text
                // transfer text to http_request
            }
